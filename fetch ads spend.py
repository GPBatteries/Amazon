...
def get_campaign_names(access_token: str) -> dict:
    """
    Haalt campaignId -> campaignName op via de (snelle, niet-asynchrone)
    Campaigns-lijst-API, in plaats van deze namen als kolom in het trage
    async-rapport op te vragen (Amazon-support: dat maakt rapporten traag
    of laat ze eindeloos in PENDING blijven staan). We combineren dit lokaal
    met de spend-cijfers uit het rapport.
    """
    headers = {
    ...
    while True:
        ...
        if r.status_code != 200:
            print(f"   Kon campagnelijst niet ophalen (HTTP {r.status_code}): {r.text[:200]}")
            return mapping
        data = r.json()
...
def poll_report(access_token: str, report_id: str, timeout_s: int = 12600) -> str:
    """
    Wacht tot het rapport klaar is. Geeft de download-url terug.
    Amazon-support: rapporten kunnen tot 3 uur duren, dus ruime timeout (3,5u)
    en een pol-interval van 60s (te vaak pollen kan zelf al 429's veroorzaken).
    """
...
def main():
    ...
    for day in daterange(start, end):
        ...
        try:
            ...
            row = extract_row(rows, day, campaign_mapping, campaign_names)
